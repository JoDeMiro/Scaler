...
def follow(thefile):
	...
	while True:
		...
		if c > 100:
			c = 0
			reset = True
			print('ÁÁÁÁÁÁÁÁÁÁÁÁÁÁÁÁÁÁÁÁÁÁÁÁÁÁÁÁÁÁÁÁÁÁÁÁ')
...
def main():
	...
	metriclog=open('./metric_scale_by_none.log','w', newline='')
	...
	for	line, reset in loglines: # follow the apache accesslog file
		print('\n\n\n reset =', reset, '\n\n\n')
		try:
			if first: # if script just started, initialize the necessary variables 
				...
				if cts==ts: # if the timestamp is same, not changed then keep incrementing the variables
					...
					# print('RTs = ', RTs)
					print('    gyűjtő ág')
					print('    gyűjti az adatokat')
					print('    egy időszeleten belül vagyunk és új adat jött be')
					...
					N+=1
				elif cts<ts or (ts[0:7]=="00:00:0" and cts[0:7]=="23:59:5"): # original
