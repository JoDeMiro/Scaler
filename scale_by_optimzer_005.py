...
#	metriclog=open(metric_log_file_name,'w', newline='')
	metriclog=open(metric_log_file_name,'a', newline='')
#	metriclog.write('\n')
	...
	# Amikor a feltanított tanátást használom akkor már meg van a log file és nem szabad, hogy a fejlécbe írjon
...
#	scalelog=open(scale_log_file_name,'w')
	scalelog=open(scale_log_file_name,'a')
...
# másik parám::: biztos, hogy az elmúlt 10 időpont átlagát kérem el?
					...
					# if( rnd > 0.5 ): # véletlenszerűen skáláz fel, vag le.
                        ...
                        # New AI Scaler
                        # >>>> számoja ki hogy melyik lenne az a k amivel vissza hozná a sávba
                        
						printTest('    >>>>>>>>>> NEW AI FEEEEELLL >>>>>>>>>>')
                        
						optimizer.load_data()
						optimizer.set_input_variables()
                        
						results = optimizer.calculate_rt(-1, min_a = 0)
						print('---------------------------------------------------------------------------------------')
						results = sorted(results, key=lambda d: d['action'])
						for _ in results:
							print(_['action'], ' -> ', _['prt'])
